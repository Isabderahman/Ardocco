...
// Province configurations (focused on Casablanca & Nouaceur)
const PROVINCES: ProvinceConfig[] = [
  { name: 'Casablanca', code: 'CAS', color: '#1A7BFD' },
  { name: 'Nouaceur', code: 'NOU', color: '#60a5fa' },
  { name: 'Mohammedia', code: 'MOH', color: '#94a3b8' },
  { name: 'Benslimane', code: 'BEN', color: '#cbd5e1' },
  { name: 'Settat', code: 'SET', color: '#e2e8f0' },
  { name: 'El Jadida', code: 'JDI', color: '#cbd5e1' },
  { name: 'Berrechid', code: 'BER', color: '#94a3b8' },
  { name: 'Mediouna', code: 'MED', color: '#64748b' },
  { name: 'Sidi Bennour', code: 'SBN', color: '#e2e8f0' }
]
...
function initializeMap(leaflet: LeafletNamespace): LeafletMap {
  ...
  // Create map centered on Casablanca & Nouaceur
  map = leaflet
    ...
    .setView([33.3675, -7.5898], props.zoom)

  // Add minimalist grayscale tiles (CartoDB Positron)
...
watch(
  ...
  (id, oldId) => {
    // Hide province boundaries when a terrain is selected
    if (id && !oldId && props.showProvinceBoundaries && L && map) {
      clearProvinceBoundaries(L, map)
    }
    // Show province boundaries when terrain is deselected
    if (!id && oldId && props.showProvinceBoundaries) {
      void reloadActiveProvinces({ fitBounds: false })
    }
