...
function getLatestEtude(listing: BackendListing): EtudeInvestissement | null {
  const etudes = listing.etudesInvestissement
  if (!Array.isArray(etudes) || etudes.length === 0) return null
  return etudes[0]
}

function getEtudeBadge(etude: EtudeInvestissement | null): { label: string; color: string } | null {
  if (!etude) return null
  const map: Record<string, { label: string; color: string }> = {
    draft: { label: 'Étude: Brouillon', color: 'bg-gray-100 text-gray-600' },
    pending_review: { label: 'Étude: En attente', color: 'bg-yellow-100 text-yellow-700' },
    approved: { label: 'Étude: Approuvée', color: 'bg-green-100 text-green-700' },
    rejected: { label: 'Étude: Rejetée', color: 'bg-red-100 text-red-700' }
  }
  return map[etude.status] || null
}
        ...
        <div class="flex items-start justify-between gap-4">
          <div class="flex-1">
            <div class="flex items-center gap-2 mb-2">
              ...
              <span
                v-if="getEtudeBadge(getLatestEtude(listing))"
                class="text-xs px-2 py-0.5 rounded-full"
                :class="getEtudeBadge(getLatestEtude(listing))?.color"
              >
                {{ getEtudeBadge(getLatestEtude(listing))?.label }}
              </span>
            </div>
            ...
            <div class="flex items-center gap-6 mt-4 text-sm">
              ...
              <span
                v-if="getLatestEtude(listing)?.ratio"
                class="font-semibold"
                :class="(getLatestEtude(listing)?.ratio ?? 0) > 0 ? 'text-green-600' : 'text-red-600'"
              >
                Ratio: {{ getLatestEtude(listing)?.ratio?.toFixed(1) }}%
              </span>
            </div>
